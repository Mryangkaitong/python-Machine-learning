...
class PGTSVParser:
  ...
  def parse_line(self, line):
    ...
    if len(attribs) != len(list(self.fields)):
      raise ValueError("Expected %(num_rows_declared)d attributes, but found %(num_rows_found)d in input row:\n%(row)s" % dict(
        num_rows_declared=len(list(self.fields)), num_rows_found=len(attribs), row=row,
      ))
...
class PGTSVPrinter:
  ...
  def write(self, out):
    if len(out) != len(list(self.fields)):
      raise ValueError("Expected %(num_rows_declared)d attributes, but found %(num_rows_found)d in output row:\n%(row)s" % dict(
        num_rows_declared=len(list(self.fields)), num_rows_found=len(out), row=out,
      ))
    else:
