...
def extract(
    ...
    for index,sent in enumerate(list(sents)):
        ...
        #去重
#        windows_maxsize = 10
#        for windows_size in range(1,windows_maxsize):
#            length = len(tokens)
#            for start_index in range(length):
#                end_index = start_index+windows_size
#                while end_index+windows_size<=length:
#                    if tokens[start_index:start_index+windows_size]==tokens[end_index:end_index+windows_size]:
#                        tokens = tokens[:start_index+windows_size]+tokens[end_index+windows_size:]
#                        length = len(tokens)
#                    else:
#                        break                    
        #lemmas：词元
